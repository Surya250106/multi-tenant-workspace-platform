...
async function run() {
  try {
    ...
    socket.on("connect", () => {
      console.log("CONNECTED TO GATEWAY. Sending auth packet...");
    ...
    socket.on("user_joined", (data) => {
      console.log("EVENT RECEIVED - USER JOINED PRESENCE:", data);
    });

    socket.on("user_left", (data) => {
      console.log("EVENT RECEIVED - USER LEFT PRESENCE:", data);
    });
    ...
    console.error("Error running test-socket-auth script:", err.message);
    process.exit(1);
  }
}
